  ...
  return (
    <div className='main-nav'>
      ...
      <div className='contact'>
        <NavItem onClick={ props.toggle } style={NavStyle} >Contact</NavItem>
      </div>
      { props.modalState ? <Popup scrollTo={section => scrollTo(section)} toggle={props.toggle} page={props.page}/> : null }
      <div className='icon'>
        <span className='bars' onClick={ props.toggle }>
          <FontAwesomeIcon icon={faBars} size='1x' color={`${currentTheme.pageColor}`} />
        </span>
      </div>
    </div>
  );
